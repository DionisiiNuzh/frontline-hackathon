...
The interface now uses a staged two-column dispatcher layout. The active-call stage places the transcript left and incident picture right, with three equal-weight “Ask next” prompts in a large, high-contrast block. Sending the review-only message reveals and smoothly scrolls to a response stage with ranked teams left and a UK operational schematic right. Seeded teams are mapped to named, plausible UK bases and the North Ridge scenario is highlighted in the Lake District area. The schematic is not live tracking and no external dispatch occurs.
